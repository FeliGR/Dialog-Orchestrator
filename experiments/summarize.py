    ...
    for condition_dir in runs_dir.iterdir():
        ...
        for json_file in condition_dir.glob("*.json"):
            try:
                ...
                # Calculate trait means from results
                trait_totals = {"O": [], "C": [], "E": [], "A": [], "N": []}
                valid_responses = 0
                unknown_responses = 0
                
                for result in data.get("results", []):
                    response = result.get("response", "UNK")
                    if response == "UNK" or response == "UNKNOWN":
                        unknown_responses += 1
                    else:
                        valid_responses += 1
                ...
                for trait, scores in trait_totals.items():
                    trait_means[f"{trait}_mean"] = statistics.mean(scores) if scores else 0
                
                # Calculate quality metrics
                total_responses = valid_responses + unknown_responses
                unk_rate = unknown_responses / total_responses if total_responses > 0 else 0
                        ...
                        with open(md_file) as f:
                            ...
                            for line in content.split('\n'):
                                if 'Correlation with persona:' in line:
                                    ...
                                    try:
                                        mae_val = line.split('MAE:')[1].split(',')[0].strip()
...
def write_summary_csv(results: List[Dict[str, Any]], output_file: Path):
    ...
    # Define column order
    columns = [
        "condition", "seed", "order", "valid_responses", "total_responses", 
        "unk_rate", "O_mean", "C_mean", "E_mean", "A_mean", "N_mean",
        "correlation", "mae", "rmse", "duration_seconds", "avg_latency_ms", 
    ...
    with open(output_file, 'w', newline='') as f:
        ...
        sorted_results = sorted(results, key=lambda x: (x['condition'], x['seed'], x['order']))
        
        for row in sorted_results:
            # Ensure all numeric fields are present
            for col in columns:
                if col not in row:
                    row[col] = 0 if col.endswith(('_mean', '_rate', '_seconds', '_ms', '_tokens', 'correlation', 'mae', 'rmse')) else ""
            writer.writerow(row)
