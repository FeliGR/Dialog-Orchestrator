...
def check_monotonicity(data: List[Dict]) -> Tuple[bool, str]:
    ...
    for condition in available:
        ...
        if prev_val is not None:
            if curr_val <= prev_val + THRESHOLDS["monotonicity_tolerance"]:
                violations.append(
                    f"{prev_condition}({prev_val:.3f}) >= {condition}({curr_val:.3f})"
                )
        prev_val = curr_val
    ...
    return (
        True,
        f"PASS: Monotonicity OK - {' < '.join(f'{c}({e_means[c]:.3f})' for c in available)}",
    )
...
def check_unk_rates(data: List[Dict]) -> Tuple[bool, str]:
    ...
    if high_unk:
        return (
            False,
            f"FAIL: High UNK rate - {', '.join(high_unk)} > {THRESHOLDS['unk_rate_max']}",
        )
...
def check_sensitivity_analysis(runs_dir: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        r2 = sensitivity.get("r_squared", sensitivity.get("r2", 0))
        ...
        if slope_deviation > THRESHOLDS["sensitivity_slope_tolerance"]:
            issues.append(
                f"|slope-1|={slope_deviation:.3f} > {THRESHOLDS['sensitivity_slope_tolerance']}"
            )
