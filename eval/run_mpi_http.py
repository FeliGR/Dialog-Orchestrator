...
class MPIHTTPRunner:
    ...
    def run_assessment(
        ...
        config = {"temperature": 0.0, "top_p": 1.0, "max_tokens": 1, "stop": ["\n"]}
        ...
        for i, item in enumerate(items_to_process, 1):
            ...
            for attempt in range(1, 3 if retry_unk else 2):  # Max 2 attempts
                try:
                    print(
                        f"Processing item {i}/{len(items_to_process)}: {item.get('label_raw', 'unknown')} (attempt {attempt})"
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        if result.get("status") == "success":
                            ...
                            result_entry = {
                                ...
                                "prompt_tokens": int(
                                    data.get("meta", {}).get("prompt_tokens", 0)
                                ),
                                "completion_tokens": int(
                                    data.get("meta", {}).get("completion_tokens", 0)
                                ),
                                # Assessment configuration
                            ...
                            if parsed_choice == "UNK":
                                ...
                                stats["successful"] += 1
                                print(
                                    f"  → {parsed_choice} ({result_entry['latency_ms']}ms)"
                                )
        ...
        print(
            f"Completed assessment: {stats['successful']}/{stats['total_items']} items successful"
        )
        print(
            f"Unknown responses: {stats['unknown']}, Retries: {stats['retried']}, Errors: {stats['errors']}"
        )
        print(
            f"Total duration: {total_duration:.1f}s, Avg latency: {metadata['avg_latency_ms']:.0f}ms"
        )
...
def main():
    ...
    parser.add_argument(
        "--format-id", default="MPI-120", help="Format identifier (default: MPI-120)"
    )
    parser.add_argument(
        "--item-order-seed", type=int, help="Seed for randomizing item order"
    )
    parser.add_argument(
        "--no-retry-unk", action="store_true", help="Disable retrying unknown responses"
    )
